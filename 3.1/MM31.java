...
public class MM311 {
    ...
    public static int mancheHumain(int lgCode, char[] tabCouleurs, int numManche, int nbEssaisMax) {
        ...
        int[] res = new int[2];
        int[][] resHist = new int[nbEssaisMax][2];
        String codeProp;
        int[][] propHist = new int[nbEssaisMax][lgCode];
        while (res[0] != cod.length && nbCoups <= nbEssaisMax) {
            System.out.println("Manche " + numManche);
            System.out.println("Coups : " + (nbCoups+1) + " / " + nbEssaisMax);
            affichePlateau(propHist, resHist, nbCoups, tabCouleurs);
            System.out.print("Couleurs disponibles : ");
            for (int i = 0; i < tabCouleurs.length; i++) {
                System.out.print(tabCouleurs[i] + ", ");
            }
            System.out.println("\nProposez un code" + " de " + cod.length + " couleurs");
            codeProp = Ut.saisirChaine().toUpperCase();
            while (!codeCorrect(codeProp, lgCode, tabCouleurs) || codeProp.length() != cod.length) {
                System.out.println("Code incorrect, ressaisir un nouveau code");
                codeProp = Ut.saisirChaine().toUpperCase();
            }
            res = nbBienMalPlaces(cod, motVersEntiers(codeProp, tabCouleurs), tabCouleurs.length);
            resHist[nbCoups]= res;
            propHist[nbCoups] = motVersEntiers(codeProp, tabCouleurs);
        ...
        if (nbCoups < nbEssaisMax) {
            ...
            System.out.println("Vous avez perdu, le code était : " + entiersVersMot(cod, tabCouleurs));
    ...
    public static int mancheOrdinateur(int lgCode, char[] tabCouleurs, int numManche, int nbEssaisMax) {
        ...
        while(nbCoups < nbEssaisMax) {
            System.out.println("Manche " + numManche);
            System.out.println("Coups : " + nbCoups + " / " + nbEssaisMax);
            affichePlateau(cod, rep, nbCoups, tabCouleurs);
            System.out.print("L'ordinateur propose: ");
            for (int i : cod1) {
                System.out.print(tabCouleurs[i] + " ");
            }
            System.out.println();
            rep[nbCoups] = copieTab(reponseHumain(lgCode));
            ...
            nbCoups++;
            if (!passeCodeSuivantLexicoCompat(cod1, cod, rep, nbCoups, nbCouleurs)) {
                System.out.println("\nAucun code n'est compatible avec vos réponses");
            }
            Ut.clearConsole();
        }
        System.out.println("\nLa machine a dépassé le nombre d'essais maximum.");
    ...
    */
    ...
    public static void affichePlateau(int[][] cod, int[][] rep, int nbCoups, char[] tabCouleurs) {
        for (int i = 0; i < nbCoups; i++) {
            for (int k : cod[i]) {
                System.out.print(tabCouleurs[k] + " ");
            }
            System.out.print(" | " + rep[i][0] + " " + rep[i][1] + "\n");
        }
    }
    // ___________________________________________________________________
